...
#define FMFPCEE_IM_MASK_RXF	0x80000000 /*Rx frame interrupt event Mask*/
...
struct ccsr_fman_t {
	u8			muram[0x80000];
	struct  fm_bmi_common_t	fm_bmi_common;
	struct  fm_qmi_common_t	fm_qmi_common;
	u8			res0[2048];
	struct {
		struct  fm_bmi_t		fm_bmi;
		struct  fm_qmi_t		fm_qmi;
		struct  fm_parser_t	fm_parser;
		u8			res[1024];
	} port[63];
	struct  fm_policer_t	fm_policer;
	struct  fm_keygen_t	fm_keygen;
	struct  fm_dma_t		fm_dma;
	struct  fm_fpm_t		fm_fpm;
	struct  fm_imem_t	fm_imem;
	u8			res1[8*1024];
	struct  fm_soft_parser_t	fm_soft_parser;
	u8			res2[96*1024];

	struct {
		struct  fm_memac_t	fm_memac;
		struct  fm_memac_mdio_t	fm_memac_mdio;
	} memac[10];
	u8			res4[32*1024];
	struct  fm_memac_mdio_t	fm_dedicated_mdio[2];

	struct  fm_1588_t	fm_1588;
	u8			res5[4*1024];
};
