...
error_alloc_chrdev:
	unregister_chrdev_region(dev, FSL_MC_BUS_MAX_MINORS);
error_class_create:
	return error;
}
...
int fsl_mc_restool_create_device_file(struct fsl_mc_bus *mc_bus)
{
	...
	if (WARN_ON(mc_restool))
		return -EINVAL;

	mc_restool = &mc_bus->restool_misc;
	mc_restool->dev = MKDEV(fsl_mc_bus_major, fsl_mc_bus_minor++);
	cdev_init(&mc_restool->cdev, &fsl_mc_restool_dev_fops);

	rc = cdev_add(&mc_restool->cdev, mc_restool->dev, 1);
	if (rc)
		goto error_cdev_add;
...
{
	if (WARN_ON(mc_restool != &mc_bus->restool_misc))
		return;
