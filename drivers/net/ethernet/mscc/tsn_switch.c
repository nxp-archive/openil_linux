...
#define MSCC_MIN(x, y) ((x) < (y) ? (x) : (y))
#define MSCC_MAX(x, y) ((x) > (y) ? (x) : (y))

#define MSCC_STREAM_HANDLE_NUM \
		MSCC_MAX(MSCC_PSFP_SFID_NUM, MSCC_FRER_SSID_NUM)
			  ...
			  struct tsn_qbv_status *qbvstatus)
{
	...
	qbvstatus->config_pending =
		(val & QSYS_PARAM_STATUS_REG_8_CONFIG_PENDING) ? TRUE : FALSE;
			       ...
			       u8 percent)
{
	...
	cir = MSCC_MIN(GENMASK(15, 0), cir);
	cbs = MSCC_MIN(GENMASK(6, 0), cbs);
...
int switch_port_shaper_set(struct net_device *ndev, u8 tc, u8 bw)
{
	...
	weight[tc] = bw;
	for (i = 0; i <= capa.qos_cos_max; i++) {
		if (weight[i])
			/* Find the lowest weight */
			w_min = MSCC_MIN(w_min, weight[i]);
	}
	for (i = 0; i <= capa.qos_cos_max; i++) {
		if (weight[i]) {
			u32 c = (((c_max << 4) * w_min / weight[i]) +
				 8) >> 4;
				      ...
				      u8 fwdmask)
{
	...
	if (i < MSCC_NUM_OUT_PORT) {
		...
		if (n) {
			dst_idx = m + n;
			pgid_val = fwdmask & ((1 << MSCC_NUM_OUT_PORT) -
					      1);
		} else {
			dst_idx = fwdport;
		}
...
int switch_cb_streamid_counters_get(
		struct net_device *ndev, u32 index,
		struct tsn_cb_streamid_counters *s_counters)
{
	return 0;
}

int switch_qci_sfi_counters_get(
		struct net_device *ndev, u32 index,
		       ...
		       struct tsn_qci_psfp_sgi_conf *sgi_conf)
{
	...
	ret = readx_poll_timeout(
			sgi_read_status, ocelot, val,
			((ANA_SG_ACCESS_CTRL_CONFIG_CHANGE & val) == 0),
			10, 100000);
		       ...
		       bool enable, struct tsn_qci_psfp_fmi *fmi)
{
	...
	pir = MSCC_MIN(GENMASK(15, 0), pir);
	cir = MSCC_MIN(GENMASK(15, 0), cir);
	pbs = MSCC_MIN(pbs_max, pbs);
	cbs = MSCC_MIN(cbs_max, cbs);
