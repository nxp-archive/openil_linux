...
static int flexcan_chip_enable(struct flexcan_priv *priv)
{
	...
	while (timeout-- &&
		(flexcan_read(priv, &regs->mcr) & FLEXCAN_MCR_LPM_ACK))
		udelay(10);
...
static int flexcan_chip_freeze(struct flexcan_priv *priv)
{
	...
	while (timeout-- &&
		!(flexcan_read(priv, &regs->mcr) & FLEXCAN_MCR_FRZ_ACK))
		udelay(100);
...
static int flexcan_chip_softreset(struct flexcan_priv *priv)
{
	...
	while (timeout-- &&
		(flexcan_read(priv, &regs->mcr) & FLEXCAN_MCR_SOFTRST))
		udelay(10);
...
static int flexcan_start_xmit(struct sk_buff *skb, struct net_device *dev)
{
	...
	if (cf->can_dlc > 0) {
		...
		flexcan_write(priv, data, &regs->mb[FLEXCAN_TX_BUF_ID].data[0]);
	}
	if (cf->can_dlc > 3) {
		data = be32_to_cpup((__be32 *)&cf->data[4]);
		flexcan_write(priv, data, &regs->mb[FLEXCAN_TX_BUF_ID].data[1]);
	}

	can_put_echo_skb(skb, dev, 0);

	flexcan_write(priv, can_id, &regs->mb[FLEXCAN_TX_BUF_ID].can_id);
	flexcan_write(priv, ctrl, &regs->mb[FLEXCAN_TX_BUF_ID].can_ctrl);

	/* Errata ERR005829 step8:
	 * Write twice INACTIVE(0x8) code to first MB.
	 */
	flexcan_write(priv, FLEXCAN_MB_CODE_TX_INACTIVE,
			      ...
			      struct can_frame *cf)
{
	...
	*(__be32 *)(cf->data + 0) = cpu_to_be32(flexcan_read(priv,
							&mb->data[0]));
	*(__be32 *)(cf->data + 4) = cpu_to_be32(flexcan_read(priv,
							&mb->data[1]));
...
static irqreturn_t flexcan_irq(int irq, void *dev_id)
{
	...
	if ((reg_iflag1 & FLEXCAN_IFLAG_RX_FIFO_AVAILABLE) ||
		...
		flexcan_write(priv,
			      priv->reg_ctrl_default & ~FLEXCAN_CTRL_ERR_ALL,
			      &regs->ctrl);
...
static int flexcan_chip_start(struct net_device *dev)
{
	...
	for (i = FLEXCAN_TX_BUF_ID; i < ARRAY_SIZE(regs->mb); i++) {
		flexcan_write(priv, FLEXCAN_MB_CODE_RX_INACTIVE,
			      &regs->mb[i].can_ctrl);
	}
	...
	if (priv->devtype_data->quirks & FLEXCAN_QUIRK_DISABLE_MECR) {
		 ...
		 * MECR register
		 */
		reg_ctrl2 = flexcan_read(priv, &regs->ctrl2);
		reg_ctrl2 |= FLEXCAN_CTRL2_ECRWRE;
		flexcan_write(priv, reg_ctrl2, &regs->ctrl2);

		reg_mecr = flexcan_read(priv, &regs->mecr);
		reg_mecr &= ~FLEXCAN_MECR_ECRWRDIS;
		flexcan_write(priv, reg_mecr, &regs->mecr);
...
static int flexcan_probe(struct platform_device *pdev)
{
	...
	priv->big_endian = of_property_read_bool(pdev->dev.of_node,
					"big-endian");
