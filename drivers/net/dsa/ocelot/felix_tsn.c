...
static struct felix_switch_capa capa __ro_after_init = {
	.num_tas_gcl	= 64,
	.tas_ct_min	= 100,
	...
	.frer_his_len_max = 32,
	.qos_dscp_max	= 63,
	.qos_cos_max	= FELIX_TSN_NUM_TC - 1,
	.qos_dp_max	= 1,
};
...
static void stream_table_del(u32 index)
{
...
static struct stream_filter *stream_table_get(u32 index)
{
	struct stream_filter *tmp;
...
static int qos_shaper_conf_set(struct ocelot *ocelot, int idx,
...
static int felix_cbs_set(struct net_device *ndev, u8 tc, u8 bw)
{
	...
	qos_shaper_conf_set(ocelot, port * 8 + tc, bw, speed);
...
void felix_cbs_reset(struct ocelot *ocelot, int port, int speed)
{
	...
	for (i = 0; i < FELIX_TSN_NUM_TC; i++) {
		idx = port * 8 + i;
		if (hsch_bw[idx] > 0)
			qos_shaper_conf_set(ocelot, idx, hsch_bw[idx], speed);
	}
}
